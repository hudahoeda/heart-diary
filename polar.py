# @title 🏥 Clinical ECG Pipeline v13.2 (Added Marker Visualization)
# !pip install neurokit2 pandas matplotlib scipy
...
def load_acc_motion(acc_path, ecg_start_time, sampling_rate, n_samples):
    ...
    if acc_path is None or not os.path.exists(acc_path):
        return None, {"rest_ratio": None, "active_ratio": None}
...
def calculate_global_stats(signals, rpeaks, sampling_rate):
    """
    Calculate full comprehensive stats like the original script.
...
def analyze_rhythm(rpeaks, sampling_rate):
    ...
    if len(rr_intervals) == 0: return [], [], 0.0, np.zeros(len(rpeaks), dtype=bool)
    
    # Rolling median for adaptive threshold
    rr_series = pd.Series(rr_intervals)
    rolling_med = rr_series.rolling(window=12, center=True).median().fillna(method='bfill').fillna(method='ffill')
...
def create_ecg_plot(segment, sr, title, filename, color_hex="#8D0A1E"):
    ...
    # 3 strips of 10s = 30s total context. 
...
def check_signal_quality(segment):
    ...
    if r < 50 or r > 8000: return False
    return True

# ==========================================
# 5. MAIN
# ==========================================
def generate_clinical_pipeline(ecg_file, marker_file=None, acc_file=None):
    output_dir = "ECG_Report_Images"
    if os.path.exists(output_dir): shutil.rmtree(output_dir)
    os.makedirs(output_dir)

    # 1. Load
    _, ecg_raw, sr, start_time = load_and_sync_data(ecg_file, marker_file)
    if ecg_raw is None: return
    ...
    # Criteria: Longest segment with NO events and LOW motion
    ...
    # (This is a quick run-length encoding)
    ...
    # 7. Process Markers (Fix for missing marker visualization)
    markers_html = ""
    ...
    if df_markers is not None and not df_markers.empty:
        ...
        # Try to find timestamp column again for markers
    ...
    # Baseline Plot
    baseline_img = f"{output_dir}/Baseline.png"
    ...
    for e in sorted(all_events, key=lambda x: x['sev'], reverse=True):
        if len(unique_events) >= 5: break
        # Dedup time (don't show same event twice)
        if any(abs(e['index'] - t) < 10*sr for t in seen_times): continue
        ...
        if check_signal_quality(seg):
            ...
            seen_times.append(c)
            fn = f"{output_dir}/Event_{len(unique_events)}.png"
            time_str = (start_time + timedelta(seconds=c/sr)).strftime("%H:%M:%S")
            create_ecg_plot(seg, sr, f"{e['type']} @ {time_str}", fn)
            
            with open(fn, "rb") as f: b64 = base64.b64encode(f.read()).decode("utf-8")
            events_html += f"""
            <div class='event-box'>
    ...
    with open(baseline_img, "rb") as f: b64_base = base64.b64encode(f.read()).decode("utf-8")
    ...
    <head>
        ...
        <title>Clinical ECG Report</title>
        <style>
            :root {{
    ...
    <body>
        <div class="container">
            <h1>🏥 Clinical ECG Report <span style="font-size: 1rem; font-weight: normal; color: #666;">(Post-Ablation Diary)</span></h1>
            <p style="color: #555;">
    ...
    report_fn = "Clinical_Report.html"
    with open(report_fn, "w") as f: f.write(html)
    print(f"\n✅ Report generated: {report_fn}")

# EXECUTION
ecg_file = "/content/Polar_H10_EC4EFF22_20251202_120429_ECG.txt"
acc_file = "/content/Polar_H10_EC4EFF22_20251202_120438_ACC.txt"
marker_file = "/content/MARKER_20251202_121206.txt"  # Ensure this path is correct

if os.path.exists(ecg_file):
    generate_clinical_pipeline(ecg_file, marker_file if os.path.exists(marker_file) else None, acc_file)